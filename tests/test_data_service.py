...
class TestSetAndGetLabelsInDisplayOrder(unittest.TestCase):
    ...
    def test_simple_set_and_get(self):
        ...
        self.assertEqual(
            get_label_column_display_order(self.index_repo, self.property_repo),
            ['A', 'B', 'C', 'D'],
        )
...
class TestChangeElementOrder(unittest.TestCase):
    def test_reordering_labels(self):
        self.assertEqual(
            change_element_order(['A', 'C', 'B', 'D'], 'C', offset=1),
            ['A', 'B', 'C', 'D'],
            msg='should move C one position to the right',
        )
        self.assertEqual(
            change_element_order(['A', 'C', 'D', 'B'], 'B', offset=-2),
            ['A', 'B', 'C', 'D'],
    ...
    def test_bounds_enforcement(self):
        self.assertEqual(
            change_element_order(['D', 'A', 'B', 'C'], 'D', offset=999),
            ['A', 'B', 'C', 'D'],
            msg='should not try to move after maximum index',
        )
        self.assertEqual(
            change_element_order(['B', 'C', 'D', 'A'], 'A', offset=-999),
            ['A', 'B', 'C', 'D'],
    ...
    def test_bad_value(self):
        regex = r"'X' not found"
        with self.assertRaisesRegex(ToronError, regex):
            change_element_order(['A', 'B', 'C', 'D'], 'X', offset=1)
