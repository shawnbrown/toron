"""Tests for toron/cli/command_create.py module."""
import argparse
import os
import sys
import tempfile
...
from toron.cli import command_create
...
class TestCreate(unittest.TestCase):
    def test_create_new_file_implicit_domain(self):
        ...
        with tempfile.TemporaryDirectory(prefix='toron-') as tmpdir:
            ...
            args = argparse.Namespace(command='create', filepath=filepath, domain=None)

            self.assertFalse(os.path.exists(filepath))
            with self.assertLogs('app-toron', level='INFO') as logs_cm:
                command_create.create_file(args)  # <- Function under test.
